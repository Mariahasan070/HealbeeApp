...
public class Database extends SQLiteOpenHelper {
    ...
    public int removeItemFromCart(String username, String medicineName, String category) {
        if (username == null || username.isEmpty() || medicineName == null || medicineName.isEmpty() || category == null || category.isEmpty()) {
            Log.e("Database", "Invalid parameters for item removal");
            return 0;
        }

        SQLiteDatabase db = this.getWritableDatabase();
        // Log the query parameters for debugging
        Log.d("Database", "Removing item: username=" + username + ", medicine_name=" + medicineName + ", category=" + category);

        int rowsAffected = db.delete("cart_table", "username=? AND medicine_name=? AND category=?",
                new String[]{username, medicineName, category});
        db.close();

        if (rowsAffected > 0) {
            Log.d("Database", "Item removed from cart successfully. Rows affected: " + rowsAffected);
        } else {
            Log.d("Database", "No item found to remove. Rows affected: " + rowsAffected);
        }
